...
`;
	...
	// Function to add labels under the specified element
	function addLabelsUnderElement() {
		let intervalId;
		let timeoutId;

		// Wait for the element to be present in the DOM
		intervalId = setInterval(() => {
			const element = document.querySelector('[name="name_l70"]');
			if (element) {
				// Clear the interval and timeout once the element is found
				clearInterval(intervalId);
				clearTimeout(timeoutId);

				// Create the new div element
				const divElement = document.createElement('div');
				divElement.style.margin = '10px'; // Set the margin



				// Create and add the labels to the div
				const labels = ['فایل صوتی پادکست', 'فایل ویدیویی جلسه', 'تکالیف', 'توضیحات جلسه', 'توضیحات'];
				labels.forEach(label => {
					const labelElement = document.createElement('span');
					labelElement.textContent = label;
					labelElement.className = 'label label-primary label-inline lbmrg';

					// Add click event listener to each label
					labelElement.addEventListener('click', () => {
						// Set the label text to the input element
						element.value = label;
						// Change the label color to gray
						labelElement.className = 'label label-gray label-inline lbmrg';
					});

					divElement.appendChild(labelElement);
				});

				// Insert the new div element after the specified element
	...
	// Execute the function to add labels under the element
	addLabelsUnderElement();
